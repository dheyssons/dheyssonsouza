...
const ImageOverlay = ({
  ...
  >
    <motion.img
      src={image}
      alt=""
      className={`${sizeConfig[size].imageSize} object-cover rounded-full`}
  ...
  const createTextSegments = () => {
    ...
    return items.map((item, index) => {
      ...
      return (
        <g key={index}>
          <text
            className={`${config.fontSize} ${config.tracking} ${config.textStyle} uppercase transition-all duration-300`}
